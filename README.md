...
Right-click in an Obsidian Markdown editor and choose **Insert Badge** to preview, create, and insert multiple text badges in the order you select them. Current version: **0.2.0**.
...
The plugin interface is currently in Simplified Chinese. This guide includes the corresponding Chinese labels for plugin controls.
...
On first use, the plugin provides four example presets: 信息 (Information, blue), 完成 (Done, green), 备注 (Note, purple), and 重要 (Important, red). You can edit or delete all of them. Deliberately clearing the list does not restore the defaults.
...
The 13 automated tests cover selection order, deselection and reselection, selection snapshots, special characters, batch editor transactions, default and empty configurations, duplicates, preset creation/editing/deletion/reordering, reloading, concurrent saves, and recovery from failed saves.
...
The manifest declares a minimum Obsidian version of 1.1.1. Older versions, other themes, and mobile have not been verified through actual UI testing; the plugin currently declares desktop-only support. Older embedded browsers that do not support `color-mix()` retain the base badge background.
...
Alternatively, extract the three files from `simple-badge-0.2.0.zip` into `.obsidian/plugins/simple-badge/` in the target vault, then enable the plugin. Preserve the existing `data.json` when updating.
